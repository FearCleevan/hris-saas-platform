...
function TimeLogsTab({ onRequestCorrection }: { onRequestCorrection: (date: string) => void }) {
  ...
  return (
    <motion.div {...fadeUp(1)} className="space-y-4">
      ...
      {/* Card list — a 10-column table would be unreadable on a phone even
          with horizontal scroll; each log has few enough fields to read as
          a stacked card instead. */}
      {paged.length === 0 ? (
        <div className={`${cardClass} py-12 text-center text-gray-400 dark:text-gray-500`}>
          No records found.
        </div>
      ) : (
          ...
          {paged.map((l) => (
            <div key={l.id} className={cardClass}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white text-sm">{formatShortDate(l.date)}</p>
                  <p className="text-xs text-gray-400">{getDayName(l.date)} · {l.source}</p>
                </div>
                <StatusBadge status={l.status} />
              </div>
              <div className="grid grid-cols-4 gap-2 text-center mb-3">
                {[
                  { label: 'In', value: l.timeIn ?? '—' },
                  { label: 'Out', value: l.timeOut ?? '—' },
                  { label: 'Hours', value: l.workHours || '—' },
                  { label: 'OT', value: l.overtimeHours || '—' },
                ].map((s) => (
                  <div key={s.label}>
      ...
      <div className={`${cardClass} flex items-center justify-between`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={page === 1}
            className="min-h-11 px-3 rounded-lg text-sm font-medium border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40"
          >
            Prev
          </button>
          <button
