...
import {
  ...
  Banknote, Users, Upload, X, Plus, FileText, Paperclip, ChevronDown, Eye,
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl text-xs text-blue-700 dark:text-blue-300">
        <p>Please review all information carefully before submitting. Use <strong>Back</strong> to make corrections.</p>
        {originalData && (
          <p className="mt-1">
      ...
      <PreviewSection title={`Documents (${uploadedFiles.length} uploaded)`} icon={Upload}>
        {uploadedFiles.length === 0 ? (
          <p className="py-3 text-sm text-gray-400">No documents uploaded. You can add them later.</p>
        ) : (
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 max-w-2xl">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
            ...
            {step === 6 && (
              <div>
                ...
                <div className="flex items-center justify-between mt-6 pt-5 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <Button type="button" onClick={handleCustomNext}
                    className="flex items-center gap-1.5 bg-brand-blue hover:bg-brand-blue-dark text-white">
            ...
            {step === 7 && (
              <div>
                <PreviewStep data={allData} beneficiaries={beneficiaries} uploads={uploads} />
                <div className="flex items-center justify-between mt-6 pt-5 border-t border-gray-100 dark:border-gray-800">
                  <Button type="button" variant="outline" onClick={() => setStep((s) => s - 1)} className="flex items-center gap-1.5">
                    <ArrowLeft className="w-4 h-4" /> Back
                  </Button>
