...
// NOT auto-completion: the DO NOT AUTO-COMPLETE note below is the one
// deliberate behavior difference from the app's own UI. The app's
// updateClearanceItem()/updateFinalPayStatus() automatically call
// complete_offboarding() the moment both clearance and final pay reach
// their terminal state — meaning, in the UI, marking one last clearance
// checkbox can silently terminate an employee as a side effect. That's
// acceptable for a human clicking through a form with full context, but
// not for an MCP tool: an incidental "mark this item cleared" call should
// never itself terminate someone. This server keeps the parent
// offboarding_records.clearance_status rollup in sync (so data stays
// consistent) but requires an explicit, separately-confirmed call to
// complete_offboarding to actually complete offboarding.
export function registerOffboardingTools(server: McpServer) {
  ...
  server.tool(
    ...
    safeTool(async ({ org_id, clearance_progress_id, status, remarks, actor_email }) => {
      await assertOrgUsable(org_id)
  ...
  server.tool(
    ...
    // actor_email accepted for interface consistency with the rest of this
    // file even though this specific tool doesn't use it (plain table
    // write, no RPC/claims needed) — kept so callers don't need to
    // remember which offboarding tools take it and which don't.
    safeTool(async ({ org_id, offboarding_id, status, amount }) => {
      await assertOrgUsable(org_id)
  ...
  server.tool(
    ...
    safeTool(async ({ org_id, offboarding_id, confirm, actor_email }) => {
      await assertOrgUsable(org_id)
      if (!confirm) {
        return errorResult(
